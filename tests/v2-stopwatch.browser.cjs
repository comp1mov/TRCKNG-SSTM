...
(async()=>{
 ...
 try {
  ...
  for(const [id,type] of [['cell02','duration_min'],['cell04','duration_sec'],['cell09','duration_sec_count']]) {
   ...
   for(const [preset,expected] of [['classic','01:05'],['minutes','01min'],['hours','0.018h'],['seconds','65s'],['clock','01:05'],['last','01:05'],['classic','01:05']]) {
    await edit(id);
    ...
    await page.locator('#stopwatchPreset').selectOption(preset);await save();
    ...
    assert.match(await page.locator(`#breakdown-${id}`).textContent(),preset==='last'?/LAST/:/THIS WEEK/);
   }
  }
  ...
  await edit('cell02');await page.locator('#stopwatchPreset').selectOption('hours');await page.locator('.surface-brand').click();
  await page.locator('#surfaceDock button').click();assert.equal(await page.locator('#stopwatchPreset').inputValue(),'hours');
  await page.locator('#cellEditCancel').click();assert.equal(await page.evaluate(()=>TRCKNG_STORAGE.getItem(SstmStopwatch.key)),saved);
  await edit('cell02');assert.equal(await page.locator('#stopwatchPreset').inputValue(),'classic');await page.locator('#cellEditCancel').click();
  ...
  const running=await capture();await edit('cell02');await page.locator('#stopwatchPreset').selectOption('classic');await save();assert.equal(await capture(),running);
  await page.evaluate(()=>updateLiveDisplays());assert.equal(await page.locator('#value-cell02').textContent(),'01:05');
  ...
  // Last remains the previous completed result throughout the following run.
  await edit('cell02');await page.locator('#stopwatchPreset').selectOption('last');await save();
  await page.locator('#btn-cell02').click();await page.clock.setFixedTime(new Date(fixed.getTime()+95000));
  await page.evaluate(()=>updateLiveDisplays());
  assert.equal(await page.locator('#value-cell02').textContent(),'01:05');
  assert.equal(await page.locator('#breakdown-cell02').textContent(),'LAST · CURRENT 00:30');
  await page.locator('#btn-cell02').click();assert.equal(await page.evaluate(()=>durationStates.cell02.lastSession),30);
  assert.equal(await page.locator('#value-cell02').textContent(),'00:30');
  ...
  assert.equal(await other.locator('#value-cell02').textContent(),'00:30');await other.close();
  // Earlier explicitly saved decimal views remain available without forced conversion.
  await page.evaluate(()=>TRCKNG_STOPWATCH.set(0,'cell02','duration_min',{mode:'week',format:'minutes'}));
  const oldView=await page.evaluate(()=>TRCKNG_STORAGE.getItem(SstmStopwatch.key));
  await edit('cell02');assert.equal(await page.locator('#stopwatchPreset').inputValue(),'saved');await save();
  assert.equal(await page.evaluate(()=>TRCKNG_STORAGE.getItem(SstmStopwatch.key)),oldView);
  // A stale editor cannot replace a view received while the draft was parked.
  await edit('cell02');await page.locator('#stopwatchPreset').selectOption('hours');
  ...
  assert.equal(await page.locator('#stopwatchPreset').inputValue(),'classic');await save();
  ...
  await page.evaluate(()=>{
    ...
    TRCKNG_STOPWATCH.set(0,'cell02','duration_min',{preset:'classic'});renderHabits();
  });
  assert.equal(await page.locator('#value-cell02').textContent(),'02:00');
  ...
  for(const [width,height,lang] of [[1280,1000,'en'],[768,1024,'en'],[390,844,'ru'],[320,700,'en'],[844,390,'en']]) {
    ...
    await page.locator('#stopwatchPreset').selectOption('hours');
    await page.waitForFunction(()=>{
      const p=document.querySelector('#stopwatchPreview').getBoundingClientRect(), m=document.querySelector('#cellEditModal');
      return p.bottom<=m.querySelector('.modal-buttons').getBoundingClientRect().top-8 && p.top>=m.querySelector('.surface-panel-head').getBoundingClientRect().bottom;
    });
    for(const selector of ['#stopwatchPreset','#stopwatchPreview']) {
      const b=await page.locator(selector).boundingBox();assert.ok(b.x>=0&&b.x+b.width<=width+1);assert.ok(b.height>=44);
    }
