...
(async () => {
 ...
 try {
  ...
  await page.goto(`${base}v2/?mode=account`); await page.locator('#accountBackToField').waitFor();
  assert.equal(await page.locator('#accountBackToField').innerText(), '← К ПРИМЕРАМ');
  await page.locator('#accountBackToField').click(); await page.waitForURL('**mode=demo');
