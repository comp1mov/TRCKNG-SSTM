...
(async () => {
  ...
  try {
    ...
    assert.ok(oldKeys.includes('trckng-sstm-v2-alpha-0.9.0'));
    await old.evaluate(async () => (await navigator.serviceWorker.getRegistration('./')).update());
    await until(() => old.evaluate(async () => (await caches.keys()).includes('trckng-sstm-v1.34.8') && !(await caches.keys()).includes('trckng-sstm-v1.33.23')));
    assert.ok((await page.evaluate(() => caches.keys())).includes('trckng-sstm-v2-alpha-0.9.0'), 'Root update preserves v2 cache');
    await old.evaluate(async () => {
      const cache = await caches.open('trckng-sstm-v1.34.8');
