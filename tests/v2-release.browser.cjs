...
(async () => {
  ...
  try {
    ...
    assert.ok(oldKeys.includes('trckng-sstm-v2-alpha-0.6.1'));
    ...
    assert.ok((await page.evaluate(() => caches.keys())).includes('trckng-sstm-v2-alpha-0.6.1'), 'Root update preserves v2 cache');
    await old.evaluate(async () => {
