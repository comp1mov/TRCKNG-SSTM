...
(async () => {
  ...
  try {
    ...
    await page.locator('#stopwatchPreset').selectOption('hours'); await page.locator('#cellEditSave').click();
    ...
    assert.equal(await page.evaluate(() => JSON.parse(localStorage.getItem(TRCKNG_STORAGE.keyName)).version), 7);
    ...
    assert.deepEqual(await phone.evaluate(() => buildCloudSnapshot().stopwatchJournal.views.find(v => v.pin === 0 && v.cellId === 'cell03')), {pin:0,cellId:'cell03',preset:'hours'});
    await phone.evaluate(() => openCellEditModal('cell03')); await phone.locator('#stopwatchPreset').selectOption('clock'); await phone.locator('#cellEditSave').click();
    ...
    assert.equal(await page.evaluate(() => buildCloudSnapshot().stopwatchJournal.views.find(v => v.pin === 0 && v.cellId === 'cell03').preset), 'clock');
    ...
    assert.equal(exported.value.schemaVersion, 5); assert.equal(exported.value.stopwatchJournal.views.find(v => v.pin === 0 && v.cellId === 'cell03').preset, 'clock');
