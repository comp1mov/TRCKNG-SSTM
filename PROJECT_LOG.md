...
Published release 0.9.2: existing Unit settings now offer Count (default) or Time since last mark, plus optional tap confirmation. The elapsed face shows minutes/seconds below one hour, hours/minutes below 48 hours, then completed days with EN/RU units. The existing weekly/all-weeks count stays underneath; previous-week preview retains historical counts. Positive marks renew the baseline; decreases and zero-change taps do not. Numerical undo restores the previous baseline, including null. Unknown legacy recency remains unknown unless retained positive history supplies it.
