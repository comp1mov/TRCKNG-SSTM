...
TRCKNG SSTM v1.33.0 uses Supabase as an optional cloud layer. The app stays local-first: without Supabase config it continues to store everything in browser `localStorage`.
...
The Supabase dashboard may suggest `npm install @supabase/supabase-js @supabase/ssr` for Next.js. TRCKNG SSTM v1.33.0 is still a static app, so it uses the browser CDN build of `@supabase/supabase-js@2`; `@supabase/ssr` is not needed until the project moves to a server-rendered framework.
...
v1.33.0 keeps manual sync controls, but signed-in local changes are now queued for debounced autosync. The main-screen `SYNC` button runs a pull-first check, then uploads pending local changes only when no conflict is detected. Conflict handling remains conservative: automatic upload pauses when both cloud and local data may differ. Counter/value change logs are part of the same full-app snapshot; no extra Supabase table is required.
