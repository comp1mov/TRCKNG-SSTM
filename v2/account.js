...
(() => {
  ...
  async function sync() {
    ...
    try {
      ...
      if (!row) { state(ready ? 'Облачная копия не найдена. Локальные записи сохранены.' : 'Подготовь независимую копию v2. V1 продолжит работать отдельно.'); return; }
      ...
      state('V2 СИНХРОНИЗИРОВАНА · V1 ОТДЕЛЬНО', 'ok');
  ...
  function installUI() {
    ...
    gate.innerHTML = '<strong>V2 · ОТДЕЛЬНОЕ ПРОСТРАНСТВО</strong><p id="migrationMessage">Подключаем аккаунт…</p><p>Перенос скопирует все три PIN и историю из облачной v1. Затем версии будут жить независимо. Исходная копия останется доступна.</p><div id="migrationSummary"></div><div class="migration-actions"><button id="migrationPreview" type="button">ПОДГОТОВИТЬ КОПИЮ V1</button><button id="migrationApply" type="button" hidden>СОЗДАТЬ МОЮ V2</button><button id="migrationRetry" type="button">ПРОВЕРИТЬ СНОВА</button><a href="../">ОТКРЫТЬ V1 ↗</a></div>';
    ...
    $('accountModal').firstElementChild.append(backup);
    $('accountUpload').hidden = true; $('accountLoad').textContent = 'ЗАГРУЗИТЬ СОХРАНЁННУЮ V2';
    $('accountConfigBody').hidden = true; $('accountConfigToggle').hidden = true; $('accountSignUp').hidden = true;
    ...
    $('accountModal').firstElementChild.append(recovery);
  }
  ...
  const api = {
    ...
    signUp() { state('Для пилотной v2 используй существующий аккаунт v1.'); },
